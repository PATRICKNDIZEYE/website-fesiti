...
export default function MessagesPage() {
  ...
  const fetchConversations = async () => {
    try {
      setLoading(true)
      const response = await orgApi.get(orgId, 'messages/conversations')
  ...
  const fetchAvailableUsers = async () => {
    try {
      const response = await orgApi.get(orgId, 'messages/users')
  ...
  const handleStartConversation = async (userId: string) => {
    try {
      const response = await orgApi.post(orgId, 'messages/conversations', {
      ...
      // Silently handle errors - user might not belong to organization
      // The user list should already be filtered, but if there's an error, just log it
    }
  }
  ...
  const fetchMessages = async (conversationId: string, showLoading = true) => {
    try {
      ...
      const response = await orgApi.get(orgId, `messages/conversations/${conversationId}/messages`, {
        params: { limit: 100 },
      })
  ...
  const handleSendMessage = async () => {
    ...
    try {
      ...
      // Use regular api for multipart/form-data with orgId in URL
      const apiUrl = process.env.NEXT_PUBLIC_API_URL || 'http://localhost:3001'
      await api.post(`${apiUrl}/org/${orgId}/messages/conversations/${selectedConversation.id}/messages`, formData, {
        headers: {
  ...
  const getFileUrl = (filePath: string) => {
    const apiUrl = process.env.NEXT_PUBLIC_API_URL || 'http://localhost:3001'
    return `${apiUrl}/org/${orgId}/messages/files/${encodeURIComponent(filePath)}`
  }
