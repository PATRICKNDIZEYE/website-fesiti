...
export default function ProjectVisualizationPage() {
  ...
  const fetchProject = async () => {
    try {
      const response = await orgApi.get(orgId, `projects/${projectId}`)
  ...
  const generateShareLink = async () => {
    try {
      ...
      const response = await orgApi.post(orgId, `dashboard/projects/${projectId}/share`, { config })
  ...
  const fetchShareLinks = async () => {
    try {
      const response = await orgApi.get(orgId, `dashboard/projects/${projectId}/share`)
  ...
  const deleteShareLink = async (shareId: string) => {
    ...
    try {
      await orgApi.delete(orgId, `dashboard/share/${shareId}`)
      await fetchShareLinks()
    } catch (error) {
    ...
    try {
      ...
      switch (chart.chartType) {
        ...
        case 'completion-status':
          const summaryResponse = await orgApi.get(orgId,
            `dashboard/projects/${projectId}/progress-summary`
          )
          data = summaryResponse.data
          break
        case 'target-vs-actual':
          const comparisonResponse = await orgApi.get(orgId,
        ...
        case 'monthly-comparison':
          const timelineResponse = await orgApi.get(orgId,
